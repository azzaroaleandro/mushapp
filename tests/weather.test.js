...
test('API URL uses Rome, history and explicit wind units',()=>{
 const url=new URL(weatherUrl());assert.equal(url.searchParams.get('past_days'),'35');assert.equal(url.searchParams.get('timezone'),'Europe/Rome');assert.equal(url.searchParams.get('latitude').split(',').length,6);
});
...
test('batch adapter retains partial success and fails on wrong area count',async()=>{
 ...
 assert.equal(Object.keys(bundle.data).length,5);assert.equal(Object.keys(bundle.errors).length,1);
...
test('rate limiting and invalid cache never masquerade as fresh data',async()=>{
 ...
 assert.equal(usableCache({version:1,date:TODAY,data:{},fetchedAt:TODAY+'T09:45:00Z'},now),true);
 assert.equal(usableCache({version:1,date:TODAY,data:{},fetchedAt:TODAY+'T09:00:00Z'},now),false);
});
