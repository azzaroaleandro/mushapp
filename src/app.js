import {initCatalog} from './catalog.js?v=0.4.1';
...
function renderStatus(){
 ...
 if(state.loading){el.textContent='Leggo le ultime settimane e le previsioni per le aree pilota…';return;}
...
function renderMap(rows){
 ...
 rows.forEach(({area,result},i)=>{
  ...
  popup.innerHTML='<strong>'+area.name+'</strong><br>'+REGIONS[area.region]+' · '+(PROVINCES.find(p=>p.id===provinceFor(area))?.name??'')+' · area indicativa<br><span>'+result.label+'</span><br><button class="button secondary" type="button">Dettagli della zona</button>';
...
function route(){
 ...
 $('#page-label').textContent={explore:'Esplora i boschi',calendar:'Le stagioni',catalog:'Catalogo dei funghi',journal:'Il mio diario',method:'Come funziona'}[view];
...
$('#region').value=state.region;renderProvinces();renderDates();renderMethod();initCatalog();renderZones();renderJournal();route();initMap();loadWeather();
